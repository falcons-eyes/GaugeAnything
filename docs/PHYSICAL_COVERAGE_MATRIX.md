...
| official | 7 | Audited result exists and is suitable for headline use with stated caveats. |
...
| Known Object Scale / Documents | `document_card_scale` | document/card metric scale | mm, relative % | SmartDoc15-CH1, MIDV-500 | A4 or ID-1 known-size quadrilateral | partial | SmartDoc 5k-frame A4 smoke test: naive global scale is 10.3%/14.9% p90 on height and 11.4%/17.5% p90 on width; GT quad homography is 0% by construction. | Uses SmartDoc GT quadrilaterals, not detected document masks or prompts; this is a scale-geometry diagnostic upper bound. | Replace GT quad with document detector/SAM3 prompt and evaluate MIDV-500 ID-card scale. |
