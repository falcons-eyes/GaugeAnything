...
| partial | 4 | Pipeline/result exists but has a known gate, coverage, or protocol limitation. |
...
| Counting / Dense Industrial | `rebar_count_density` | count, density, spacing | count | ROI-1555 Rebar, RebarDSC candidate | instance masks / boxes | partial | Owned density head (Count v1, 0.11M) beats zero-shot SAHI on a fair stratified split: MAE 7.0 vs 8.9, acc@10% 0.19 vs 0.075. Dense bars (GT>=40) still undercount by ~29 (MAE 30.9) — the open structural problem. | Target MAE<5 not met; dense-regime undercount persists. Block-split density shift inflates MAE to 12.0. ROI-1555 labeled images have no scene metadata, so count-stratified split is used (near-duplicate-frame leakage possible). | Count v1.1: high-res + sharp sigma + count-weighted loss for the dense bin; compare against CountGD/GeCo external counters on the same split. |
